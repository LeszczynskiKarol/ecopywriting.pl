...
BUCKET_NAME="ecopywriting-attachments"
PRESIGN_FUNCTION="ecopywriting-presign"
CONTACT_FUNCTION="ecopywriting-contact"
API_NAME="ecopywriting-api"
ROLE_NAME="ecopywriting-lambda-role"
DOMAIN="ecopywriting.pl"
...
echo "============================================"
echo "  eCopywriting.pl — Setup AWS"
echo "============================================"
...
aws s3api put-bucket-cors \
  --bucket "$BUCKET_NAME" \
  --cors-configuration '{"CORSRules":[{"AllowedOrigins":["https://ecopywriting.pl","https://www.ecopywriting.pl","http://localhost:4321"],"AllowedMethods":["PUT","GET"],"AllowedHeaders":["*"],"MaxAgeSeconds":3600}]}'
echo "    CORS OK"
...
if [ -n "$EXISTING_API_ID" ] && [ "$EXISTING_API_ID" != "None" ]; then
  ...
  API_ID=$(aws apigatewayv2 create-api \
    --name "$API_NAME" \
    --protocol-type "HTTP" \
    --cors-configuration '{"AllowOrigins":["https://www.ecopywriting.pl","https://ecopywriting.pl","http://localhost:4321"],"AllowMethods":["POST","OPTIONS"],"AllowHeaders":["Content-Type"],"MaxAge":3600}' \
    --region "$REGION" \
...
# ============================================================
# 6. SES — ecopywriting.pl juz ma SES skonfigurowany
# ============================================================
echo ">>> [6/7] SES: $DOMAIN"

# Sprawdz status — ecopywriting.pl powinno byc juz zweryfikowane
...
if [ "$SES_STATUS" = "Success" ]; then
  echo "    Domena $DOMAIN juz zweryfikowana w SES (us-east-1) ✓"
else
  echo "    UWAGA: Domena $DOMAIN NIE jest zweryfikowana w SES us-east-1"
  echo "    Sprawdz tez region eu-north-1..."

  SES_STATUS_EU=$(aws ses get-identity-verification-attributes \
    --identities "$DOMAIN" \
    --region "$REGION" \
    --query "VerificationAttributes.\"${DOMAIN}\".VerificationStatus" \
    --output text 2>/dev/null || echo "NotStarted")

  if [ "$SES_STATUS_EU" = "Success" ]; then
    echo "    Domena $DOMAIN zweryfikowana w SES ($REGION) ✓"
  else
    echo "    Weryfikuje domene w SES ($REGION)..."
...
# mail.ecopywriting.pl juz jest skonfigurowane w DNS
echo "    mail.$DOMAIN — MX i TXT juz istnieja w DNS ✓"
echo ""
...
echo ""
echo "  NASTEPNY KROK:"
echo "  W pliku src/pages/kontakt/index.astro zmien API_URL na:"
echo "  $API_URL"
echo ""